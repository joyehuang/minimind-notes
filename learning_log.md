...
### 2026-06-04：补齐网站 Architecture 模块

#### ✅ 完成事项
- [x] 补齐残差连接模块的中英文首页、教学文档、源码导读和交互式测验
- [x] 新增有/无残差训练、梯度流、深度扩展 3 个可复现实验
- [x] 将残差连接接入首页模块卡片、双语侧边栏和架构总览
- [x] 新增 `pnpm run test:content` 与 `pnpm run test:site`
- [x] 完成实验完整模式、VitePress 构建和双语浏览器验收
- [x] 补齐 Transformer Block 模块的中英文首页、教学文档、源码导读和交互式测验
- [x] 新增组件消融、组件顺序、堆叠与因果性 3 个可复现实验
- [x] 将 Transformer Block 接入首页模块卡片、双语侧边栏、架构总览和 8 小时系统学习路线

#### 🔬 实验结论
- **训练对比**：16 层残差网络最终 MSE 为 `4.54e-05`，普通网络为 `0.1300`。
- **梯度流**：48 层普通网络输入端梯度下溢为 `0`，残差网络首尾梯度比例为 `1.088`。
- **深度扩展**：64 层后残差输出与输入余弦相似度为 `0.986`，普通网络接近 `0`。
- **组件消融**：完整 Block 最终 MSE 为 `0.0148`；移除 Mixer、FFN、残差后分别恶化约 `2.14x`、`1.63x`、`67.81x`。
- **组件顺序**：交换 Mixer 与 FFN 后，输出平均绝对差异为 `0.1295`，说明顺序会改变 Block 函数。
- **堆叠与因果性**：2、8、16 层 Block 均保持输入形状；修改未来 token 后，过去位置输出最大差异为 `0`，且梯度有限非零。

#### 🐛 问题与解决方案
- **开发服务器 404**：启动命令多传了一个 `--`，改用 `pnpm exec vitepress dev --host 127.0.0.1`。
- **完整梯度测试除零**：普通网络梯度完全下溢，改为记录下溢状态并使用保守下界计算改善倍数。
- **首页缺少实验数量**：Tier 2 卡片模板未渲染实验元信息，已与 Tier 1 对齐。

---

**最后更新**：2026-06-04
**学习进度**：Tier 1 完成 ✅ + 完整架构理解 ✅ + 网站 Architecture 模块完成 ✅
...
- 2026-06-04：补齐网站 Architecture 模块并完成全套测试
